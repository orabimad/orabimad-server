var xdo = (typeof xdo === 'undefined') ? {} : xdo;
xdo.api = (typeof xdo.api === 'undefined') ? {api: {}} : xdo.api;
xdo.sdk = {plugin: {}};
// ------------------------------------------------------------
// setup API
xdo.api.getPixelValue = xdo.lang.Length.getPixelValue;
...
xdo.api.format = function (dataType, mask, formatStyle) {
  if (console)
    console.debug('[BIMAD] Formatting method called. DataType: ' + dataType + ', mask: ' + mask + ', formatStyle: ' + formatStyle);
  //Default string formatter
  if (dataType !== 'date' && dataType !== 'number' || !mask)
    return String;


  var locale = xdo.app.System.getProperty('user.locale');
  return function (value) {
    return xdo.i18n.dataformatter.DataFormatter.format(value, formatStyle, mask, dataType, locale);
  };
}
